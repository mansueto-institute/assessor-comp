...
sales_assessment_joined <- sales_assessment_joined %>%
    ...
    pivot_wider(id_cols = township_code) %>%
    right_join(sales_assessment_joined, by = join_by(township_code)) %>%
    mutate(p75_ratio = `75%`) %>%
    mutate(p25_ratio = `25%`) %>%
    mutate(ratio_iqr = p75_ratio - p25_ratio) %>%
    mutate(outlier = ifelse(sale_price < 10000, -99, 0)) %>%
    mutate(outlier = ifelse(aratio_mailed > p75_ratio + 1.5 * ratio_iqr, 1, 0)) %>%
    mutate(outlier = ifelse(aratio_mailed < p25_ratio - 1.5 * ratio_iqr, -1, 0)) %>%
