...
# av <- read_csv('data.nosync/Assessor_-_Assessed_Values.csv')
# av %>%
#   group_by(tax_year) %>%
#   write_dataset(path = 'data.nosync/Assessor_-_Assessed_Values.parquet', format = "parquet")


av_pq <- open_dataset('/Users/nm/Desktop/Projects/work/cook-assessor/commercial-shock/data.nosync/Assessor_-_Assessed_Values.parquet')

av_query <- av_pq %>%
  # to_duckdb() %>%
  filter(tax_year < 2006) %>%
  select(tax_year, pin, class, township_code, mailed_tot, certified_tot, board_tot) %>%
  collect() %>% 
...
#   group_by(tax_year, res_nonres_label) %>%
#   summarize_at(vars(mailed_tot, final_ta), list(sum), na.rm =TRUE) %>%
#   ungroup() 

ptaxsim_db_conn <- DBI::dbConnect(RSQLite::SQLite(), "data/ptaxsim-2021.0.4.db")
z <- dbGetQuery(ptaxsim_db_conn, "SELECT year, pin, tax_code_num, class, tax_bill_total, av_mailed, av_certified, av_board, av_clerk FROM pin where year = 2021") %>% as_tibble()
...
z<- z %>%
  ...
  summarize_at(vars(av_mailed, tax_bill_total), list(sum), na.rm =TRUE) %>%
  ungroup()
...
av_query <- av_query %>%
  mutate_at(vars(township_code), list(as.character)) %>%
  left_join(., townships %>% st_drop_geometry() %>% select(township_code, triad_name), by = c("township_code" = "township_code")) %>%
  mutate(av_clerk = NA,
         tax_bill_total = NA)
         
ptax_data <- rbind(ptax_data %>% select(year, pin, class, tax_bill_total, av_mailed, av_certified, av_board, av_clerk, township_code, triad_name),
                   av_query %>% select(year, pin, class, tax_bill_total, av_mailed, av_certified, av_board, av_clerk, township_code, triad_name))
...
sales_ptax <- sales_ptax %>%
  ...
  ungroup() %>%
  mutate(outlier_flag_iaao = case_when(av_ratio_mailed > lower_trim_point & av_ratio_mailed < upper_trim_point ~ 1, 
                                       TRUE ~ as.integer(0))) %>%
  group_by(year, township_code, class) %>%
  mutate(
...
# FHFA HPI Expanded-Data Index for Chicago Metro (Estimated using Enterprise, FHA, and Real Property County Recorder Data Licensed from DataQuick for sales below the annual loan limit ceiling)
...
#   as.data.frame()
hpi_2023 <- list(
  year = c(1991, 1992, 1993, 1994, 1995, 1996, 1997, 1998, 1999, 2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023),
  hpi = c(2.6898, 2.570773201, 2.504469274, 2.386055176, 2.315798536, 2.248056832, 2.19539667, 2.159961455, 2.042989518, 1.888108943, 1.755285826, 1.633151184, 1.493254871, 1.394762769, 1.262402028, 1.160597169, 1.133454132, 1.284711277, 1.601834207, 1.775328361, 1.931356358, 2.031110776, 1.952242706, 1.800642656, 1.718173108, 1.634441271, 1.51035993, 1.433566061, 1.381651942, 1.310627101, 1.197009479, 1.066280821, 1)     
) %>%
  as.data.frame()
...
sales_ptax_index <- sales_ptax %>% 
  ...
  filter(outlier_flag_iaao == 1, sale_price_outlier == 1, year >= 2000) %>%
...
sales_ptax_index <- sales_ptax_index %>%
  left_join(., sales_ptax_index %>% filter(year == 2000) %>% 
              select(sales_ntile, sale_price_hpi_avg) %>%
              rename(sale_price_hpi_avg_1999 = sale_price_hpi_avg),
            by = c('sales_ntile' = 'sales_ntile')) %>%
  mutate(sales_index = sale_price_hpi_avg/sale_price_hpi_avg_1999) %>%
...
(sales_trend <- ggplot() +
  ...
  labs(y = 'Sale price index (base = 1999)', x = 'Year', color = 'Sale price quartile',
  ...
  scale_x_continuous(name = "Tax Year", breaks = c(2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021), 
                     expand = c(0.01, .3)) +
...
sales_ptax_summary <- sales_ptax %>%
  ...
  ungroup() %>%
  mutate(
    assessed_value_board_mailed_ratio = av_board_hpi / av_mailed_hpi,
    assessed_value_board_mailed_diff = av_board_hpi - av_mailed_hpi,
    assessed_value_board_mailed_diff_pct = assessed_value_board_mailed_diff / av_mailed_hpi
  ) %>%
...
cod_note <- paste0(
  "\n\nThe coefficient of dispersion (COD) measures assessment uniformity, or horizontal equity, which means taxpayers with equal property values pay equal property taxes. COD is calculated by\n",
  "taking the average absolute percentage difference from the median sales ratio."
  )

prd_note <- paste0(
  "\n\nThe price-related differential (PRD) is a measure of regressivity, or vertical equity, which means high-value properties are appraised higher to be progressive (or lower to be regressive)\n",
  "than low-value properties in relation to their respective market values. PRD is calculated by dividing the mean ratio by the weighted mean ratio."
)

prb_note <- paste0(
  "\n\nThe coefficient of price-related bias (PRB) is a measure of vertical equity and price-related bias. PRB is calculated by regressing percentage difference from the median ratio on\n",
  "percentage differences in assessed value."
)


# Coefficient of Dispersion -----------------------------------------------

# https://www.iaao.org/media/standards/Standard_on_Ratio_Studies.pdf#page=14
# The most generally useful measure of variability or uniformity is the COD. 
# The COD measures the average percentage deviation of the ratios from the median ratio and is calculated by the following steps:
...
# Measures assessment uniformity, or horizontal equity. It is the average absolute percentage difference from the median sales ratio. 
# Horizontal equity (taxpayers with equal home values should pay equal tax)

sales_ptax_cod <- sales_ptax %>%
  # Filter using AV ratio IQR test and less than 3 st dev of sale price
  filter(year >= 2000, outlier_flag_iaao == 1, sale_price_outlier == 1) %>%
  group_by(year) %>%
  ...
  summarise(
    ...
    cod = ((sum(abs_dif) / n()) / median(av_ratio_mailed)),
    n = n()
  )

(cod_chart <- ggplot(data = sales_ptax_cod %>% filter(year >= 2011)) +
    
    geom_rect(mapping = aes(xmin = 2018.55, xmax = 2022.45, ymin = 0, ymax = 0.75), fill = "#ceccc4", color = "white", alpha = 0.5) +
    geom_rect(mapping = aes(xmin = 2014.55, xmax = 2018.45, ymin = 0, ymax = 0.75), fill = "#f6f6ed", color = "white", alpha = 0.5) +
    geom_rect(mapping = aes(xmin = 2010.55, xmax = 2014.45, ymin = 0, ymax = 0.75), fill = "#f6f6ed", color = "white", alpha = 0.5) +
    
    #geom_rect(mapping = aes(xmin = 2006.55, xmax = 2010.45, ymin = 0, ymax = 0.75), fill = "#D2D7C9", color = "white", alpha = 0.5) +
    #geom_rect(mapping = aes(xmin = 2002.55, xmax = 2006.45, ymin = 0, ymax = 0.75), fill = "#D2D7C9", color = "white", alpha = 0.5) +
    #geom_rect(mapping = aes(xmin = 1999, xmax = 2002.45, ymin = 0, ymax = 0.75), fill = "#D2D7C9", color = "white", alpha = 0.5) +
    
    geom_ribbon(aes(ymin = 0.05, ymax = 0.15, x = (year - 2016) * (11.9 / 10) + 2016.5),fill = "#49DEA4", alpha = 0.5) +
    geom_text(aes(x = 2020.5, y = 0.1, label = "COD Target")) +
    
    geom_segment(aes(x = 2021.75, xend = 2021.75, y = .12,  yend = .145), arrow = arrow(length = unit(0.2, "cm")), linewidth = .75) +
    geom_text(x = 2021.75, y = 0.15, hjust = .5, vjust = -.15, label = "Less\nuniform", size = 3.5) +
    
    geom_segment(aes(x = 2021.75, xend = 2021.75, y = .12,  yend = .055), arrow = arrow(length = unit(0.2, "cm")), linewidth = .75) +
    geom_text(x = 2021.75, y = 0.055, hjust = .5, vjust = 1.15, label = "More\nuniform", size = 3.5) +
    
    geom_line( aes(x = year, y = cod, color = "Coefficient of Dispersion"), lwd = 2 ) +
    scale_color_manual(values = "#ffc425") +
    
    #geom_text(aes(x = 2000.5, y = 0.2, label = "Assessed under\nHoulihan")) +
    #geom_text(aes(x = 2004.5, y = 0.2, label = "Assessed under\nHoulihan")) +
    #geom_text(aes(x = 2008.5, y = 0.2, label = "Assessed under\nHoulihan")) +
    ...
    scale_y_continuous(name = "Coefficient of Dispersion",  expand = c(0, 0), labels = scales::percent_format()) +
    scale_x_continuous(name = "Tax Year", expand = c(0,0),
                       #breaks = c(2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021),
                       #limits = c(1999,2023)
                       breaks = c(2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021),
                       limits = c(2010,2023)
                       ) +
    labs(subtitle = "Coefficient of Dispersion by Tax Year for residential properties", captions = paste(caption_3,cod_note) ) +
    theme_classic() +
    theme(
      legend.position = "bottom",
      plot.subtitle = element_text(size = 13, hjust = .5, face = "bold", color = "#333333"),
...
# PRD is a measure of regressivity, or vertical equity. 
# An appraisal bias such that high-value properties are appraised higher (or lower) than 
# low-value properties in relation to market values. 

# The mean divided by the weighted mean. The statistic has a slight bias upward.
...
# https://www.iaao.org/media/standards/Standard_on_Ratio_Studies.pdf#page=15

# An index statistic for measuring vertical equity is the PRD, 
# which is calculated by dividing the mean ratio by the weighted mean ratio. 
# This statistic should be close to 1.00. 
# Measures considerably above 1.00 tend to indicate assessment regressivity; 
# Measures below 1.00 suggest assessment progressivity. 
# When samples are small or the weighted mean is heavily influenced by several extreme
# sales prices, the PRD may not be a sufficiently reliable measure of vertical inequities. 
# A scatter plot of ratios versus appraised values or sale prices is a useful diagnostic tool.
...
sales_ptax_prd <- sales_ptax %>%
  ...
  summarise(
    ...
    mean_av = mean(av_ratio_mailed, na.rm = TRUE),
    #median_av = median(av_ratio_board),
    median_av = median(av_ratio_mailed),
    #weighted_mean_av = weighted.mean(av_ratio_board, sale_price_hpi),
    weighted_mean_av = weighted.mean(av_ratio_mailed, sale_price_hpi),
...
(prd_chart <- ggplot(data = sales_ptax_prd %>% filter(year >= 2011)) +
    ...
    #geom_rect(mapping = aes(xmin = 2006.55, xmax = 2010.45, ymin = 0, ymax = 2), fill = "#D2D7C9", color = "white", alpha = 0.5) +
    #geom_rect(mapping = aes(xmin = 2002.55, xmax = 2006.45, ymin = 0, ymax = 2), fill = "#D2D7C9", color = "white", alpha = 0.5) +
    #geom_rect(mapping = aes(xmin = 1999, xmax = 2002.45, ymin = 0, ymax = 2), fill = "#D2D7C9", color = "white", alpha = 0.5) +
    
    geom_ribbon(aes(ymin = 0.98, ymax = 1.03, x = (year - 2016) * (11.9 / 10) + 2016.5), fill = "#49DEA4", alpha = 0.5) +
    geom_text(aes(x =  2020.5, y = 1.01, label = "PRD Target")) +
    
    geom_segment(aes(x =  2021.75, y = 1.25, xend =  2021.75, yend = 1.9), arrow = arrow(length = unit(0.2, "cm")), linewidth = .75) +
    geom_text(x =  2021.75, y = 1.1, hjust = .5, vjust = 0, label = "More\nregressive", size = 3.8) +
    
    geom_segment(aes(x =  2021.75, y = .75, xend =  2021.75, yend = .1), arrow = arrow(length = unit(0.2, "cm")), linewidth = .75) +
    geom_text(x =  2021.75, y = 0.8, hjust = .5, vjust = 0, label = "More\nprogressive", size = 3.8) +
    
    geom_line(aes(x = year, y = prd, color = "Price-Related Differential"), lwd = 2) +
    scale_color_manual(values = "#ffc425") +
...
# A PRB of −.045 indicates, for example, that assessment ratios fall by 4.5% when values double and increase by 4.5% when values are halved. Like all regression coefficients, the statistical reliability of the PRB can be gauged by noting its t-value and related significance level, and by computing confidence intervals. 

# The coefficient of price-related bias (PRB) is an index of vertical equity that quantifies the relationship between assessment-sales ratios (ASR) and value in percentage terms. 
# A PRB of 0.043 indicates that, on average, assessment ratios increase by 4.3 percent whenever values increase by 100 percent (e.g., double or double again). 
# The PRB has several technical advantages, including being less sensitive to outliers than the PRD, and also quantifies the statistical significance of observed relationships.

# 1. Compute a value proxy, “value,” as 50 percent of sale price + 50 percent of assessed value. 
# To ensure that assessed values and sales prices receive equal weight, 
...
# 2. Take the natural logarithm of the value proxy and divide by 0.693:
# Ln_Value = ln(value)/0.693

# Taking logarithms converts the value proxy to a percentage basis, 
# which substantially minimizes the impact of atypically high values (outliers) in the analysis. 
# Dividing by 0.693 allows each increment of 1 to be interpreted as a change of 100 percent. 
# (For example, ln(100,000)/0.693 = 16.613 and ln(200,000)/0.693 = 17.613).

# 3.Compute percentage differences from the median assessment ratio 
...
# Because each increment of 1 in the independent variable represents a 100 percent change in value, 
# the regression coefficient, b1, represents the corresponding percentage change in assessment ratios.
  
#The PRB provides a measure of price-related bias that is more meaningful and less sensitive to extreme prices or ratios. 
#As a general matter, the PRB coefficient should fall between –0.05 and 0.05. 
#PRBs for which 95% confidence intervals fall outside of this range indicate that one 
#can reasonably conclude that assessment levels change by more than 5% when values are halved or doubled. 
#PRBs for which 95% confidence intervals fall outside the range of –0.10 to 0.10 indicate unacceptable 
#vertical inequities.
...
sales_ptax_prb <- sales_ptax %>%
  ...
  mutate(
    ...
    value = ((av_mailed / median(av_ratio_mailed)) + (sale_price)) / 2,
    ln_value = log(value) / log(2),
    # pct_diff = (av_ratio_board - median(av_ratio_board)) / median(av_ratio_board)
    pct_diff = (av_ratio_mailed - median(av_ratio_mailed )) / median(av_ratio_mailed)
  ) %>%
  ungroup() %>%
  select(year, pct_diff, ln_value) 

sales_ptax_prb_coeff <- sales_ptax_prb %>%
  lmList(pct_diff ~ ln_value | year, data = .) %>% #check
  coef() %>%
  rownames_to_column("year") %>%
  mutate(year = as.numeric(year))

sales_ptax_prb_intervals <- sales_ptax_prb %>%
  lmList(pct_diff ~ ln_value | year, data = .) %>% #check
  intervals(level = 0.99) 

sales_ptax_prb_intervals <- sales_ptax_prb_intervals[, , 'ln_value'] %>%
  as.data.frame() %>%
  rownames_to_column("year") %>%
  mutate(year = as.numeric(year))

sales_ptax_prb_reg <- sales_ptax_prb_coeff %>%
  left_join(., sales_ptax_prb_intervals, by = c('year'='year'))

(prb_chart <- ggplot(data = sales_ptax_prb_reg %>% filter(year >= 2011)) +
    ...
    #geom_rect(mapping = aes(xmin = 2006.55, xmax = 2010.45, ymin = -0.7, ymax = 0.16), fill = "#D2D7C9", color = "white", alpha = 0.5) +
    #geom_rect(mapping = aes(xmin = 2002.55, xmax = 2006.45, ymin = -0.7, ymax = 0.16), fill = "#D2D7C9", color = "white", alpha = 0.5) +
    #geom_rect(mapping = aes(xmin = 1999, xmax = 2002.45, ymin = -0.7, ymax = 0.16), fill = "#D2D7C9", color = "white", alpha = 0.5) +
    
    geom_ribbon(aes(ymin = -0.05, ymax = 0.05, x = (year - 2016) * (11.9 / 10) + 2016.5), fill = "#49DEA4", alpha = 0.5) +
    
    geom_text(x = 2021.5, y = 0.06, hjust = .5, vjust = 0, label = "Progressive\nbias", size = 4) +
    geom_segment(aes(x = 2021.5, y = .11, xend = 2021.5, yend = .14), arrow = arrow(length = unit(0.2, "cm")), linewidth = .75) +
    
    geom_text(x = 2021.5, y = -0.1, hjust = .5, vjust = 0, label = "Regressive\nbias", size = 4) +
    geom_segment(aes(x = 2021.5, y = -.11, xend = 2021.5, yend = -.14), arrow = arrow(length = unit(0.2, "cm")), linewidth = .75) +

    geom_line(aes(x = year, y = ln_value, color = "Coefficient of Price-Related Bias"), alpha = 1, color = '#ffc425', lwd = 2 ) +
    geom_text(aes(x = 2020.5, y = 0.01, label = "PRB Target")) +
    
    #geom_text(aes(x = 2000.5, y = -0.5, label = "Assessed under\nHoulihan")) +
    #geom_text(aes(x = 2004.5, y = -0.5, label = "Assessed under\nHoulihan")) +
    #geom_text(aes(x = 2008.5, y =- 0.5, label = "Assessed under\nHoulihan")) +
...
bg_acs_data <- get_acs(year = 2020, geography = "block group",
                      survey = 'acs5', variables = c('B19013_001'),
                      cache_table = TRUE,
                      state = '17', county = '031',
                      geometry = TRUE) %>%
...
bg_acs_data_race  <- get_acs(year = 2020, geography = "block group",
                            survey = 'acs5', variables = c('B03002_012', 'B03002_003', 'B03002_004', 'B03002_005', 'B03002_006', 'B03002_007', 'B03002_008', 'B03002_009'),
                            summary_var = 'B03002_001',
                            cache_table = TRUE,
                            state = '17', county = '031',
...
bg_acs_data_race <- bg_acs_data_race %>%
  rename_all(list(tolower)) %>%
  mutate(variable_label = case_when(variable == 'B03002_012' ~ 'Latino/a',
                                    variable == 'B03002_003' ~ 'White',
                                    variable == 'B03002_004' ~ 'Black',
  ...
  select(geoid, variable_label, share) %>%
  pivot_wider(names_from = variable_label, 
              values_from = c(share), 
              values_fill = 0) 

bg_acs_data <- bg_acs_data %>%
  left_join(., bg_acs_data_race, by = c('geoid'='geoid')) %>%
  inner_join(., 
             sales_ptax_bg %>% filter(assessor_term == "Kaegi") %>% 
               st_drop_geometry() %>% select(geoid, triad_name),
             by = c('geoid'='geoid'))
...
(race_demos_map <- ggplot() +
  geom_sf(data = bg_acs_data %>% filter(triad_name == triad_label), aes(fill = Black), color = "white", linewidth = .1) +
  labs(subtitle = "Black") +
  scale_fill_viridis(limits = c(0,1), labels = percent_format(), name = '') +
  theme_void() + theme(plot.subtitle = element_text(hjust = .5, face = 'bold')) +
ggplot() +
  geom_sf(data = bg_acs_data %>% filter(triad_name == triad_label), aes(fill = White), color = "white", linewidth = .1) +
  labs(subtitle = "White") +
  scale_fill_viridis(limits = c(0,1), labels = percent_format(), name = '') +
  theme_void() + theme(plot.subtitle = element_text(hjust = .5, face = 'bold')) +
ggplot() +
  geom_sf(data = bg_acs_data %>% filter(triad_name == triad_label), aes(fill = `Latino/a`), color = "white", linewidth = .1) +
  labs(subtitle = "Latino/a") +
  scale_fill_viridis(limits = c(0,1), labels = percent_format(), name = '') +
  theme_void() + theme(plot.subtitle = element_text(hjust = .5, face = 'bold')) +
ggplot() +
  geom_sf(data = bg_acs_data %>% filter(triad_name == triad_label), aes(fill = Asian), color = "white", linewidth = .1) +
  labs(subtitle = "Asian") +
  scale_fill_viridis(limits = c(0,1), labels = percent_format(), name = '') +
  theme_void() + theme(plot.subtitle = element_text(hjust = .5, face = 'bold')) +
...
#ggsave(plot = race_demos_map, filename = paste0(viz_dir,"/16-",triad_label,"_race_map.pdf"), width = 10, height = 14)
#ggsave(plot = race_demos_map, filename = paste0(viz_dir,"/16-",triad_label,"_race_map.pdf"), width = 14, height = 10)
