...
# curl::multi_download("https://datacatalog.cookcountyil.gov/api/views/uzyt-m557/rows.csv", "data/Assessor_-_Parcel_Sales.csv", resume = TRUE)
...
class_data <- class_data %>%
  mutate(res_nonres_label = case_when(major_class_type == 'Residential' ~ 'Residential',
...
sales_ptax <- sales_ptax %>%
  mutate(av_ratio_mailed = av_mailed / sale_price,
...
sales_ptax <- sales_ptax %>% 
  ...
  mutate(quartile_1 = quantile(x = av_ratio_mailed, 1/4), 
  ...
  group_by(year, township_code, class) %>%
  mutate(av_ratio_1_to_99 = case_when(av_ratio_mailed < quantile(av_ratio_mailed, 0.01) | av_ratio_mailed > quantile(av_ratio_mailed, 0.99) ~ 0, TRUE ~ as.integer(1)),
...
housing_cpi_2023 <- list(year = c(2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023),
...
case_shiller_2023 <- list(year = c(2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023),
...
hpi_2023 <- list(year = c(2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023),
...
sales_ptax <- sales_ptax %>% 
  mutate(across(all_of(c("tax_bill_total", "sale_price", "av_mailed", "av_certified", "av_board", "av_clerk")), ~ .x * cpi, .names = "{.col}_cpi"),
         ...
         across(all_of(c("tax_bill_total", "sale_price", "av_mailed", "av_certified", "av_board", "av_clerk")), ~ .x * case_shiller, .names = "{.col}_hpi"),
...
sales_ptax <- sales_ptax %>% 
  ...
  mutate(assessor_term = case_when(year <= 2010 & year >= 1998 ~ 'Houlihan',
                                   year <= 2018 & year >= 2011 ~ 'Berrios',
                                   year <= 2024 & year >= 2019 ~ 'Kaegi'),
         comparison_years = case_when(year == 2018 | year == 2021 ~ '2018 vs. 2021', # city
...
sales_ptax_summary <- sales_ptax %>% 
  ...
  ungroup() %>%
  mutate(count = 1) %>%
  group_by(sales_ntile, reporting_group, 
...
(cook_comp_av <- ggplot(data = sales_ptax_summary,
                        aes(x = sale_price_hpi, y = av_ratio_mailed, 
    ...
    labs(subtitle = 'Cook County', 
         caption = paste0('Note: Points represent averages summarized by sales percentile, property type, triad, and year. Points are scaled to the number of sales. Ratios more than 1.5 times the lower or upper\n',
    ...
    theme_classic() + 
    theme(plot.subtitle = element_text(size = 15, hjust = .5, face = 'bold', color = '#333333'),
...
sales_ptax_summary <- sales_ptax %>% 
  ...
  ungroup() %>%
  mutate(count = 1) %>%
  group_by(sales_ntile, reporting_group, triad_name, 
           comparison_years, assessor_term, assessor_term_year) %>%
  summarize(count = sum(count), 
            av_ratio_mailed = mean(av_ratio_mailed), 
            sale_price = mean(sale_price), 
...
(city_comp_av <- ggplot(data = sales_ptax_summary %>% filter(triad_name == 'City' & comparison_years == '2018 vs. 2021'),
                        aes(x = sale_price_hpi, y = av_ratio_mailed, 
    ...
    scale_x_continuous(name = 'Sale price', expand = c(0,20), limits = c(0, 1000000), breaks = c(0,250000,500000,750000,1000000), labels = c('','$250K','$500K','$750K','$1M')) +
    scale_size_binned(name = 'Number of sales', range = c(0, 3), n.breaks = 3, breaks = waiver(), labels = comma_format()) +
    labs(subtitle = 'City of Chicago', 
         caption = paste0('Note: Points represent averages summarized by sales percentile, property type, triad, and year. Points are scaled to the number of sales. Ratios more than 1.5 times the lower or upper\n',
                          'interquartile range were excluded. Sale prices are inflation adjusted to 2023 dollars using the FHFA HPI.')) +
    theme_classic() + 
    theme(plot.subtitle = element_text(size = 15, hjust = .5, face = 'bold', color = '#333333'),
          plot.caption = element_text(size = 12, hjust = 0),
          legend.key.width=unit(40,"pt"),
...
(north_comp_av <- ggplot(data = sales_ptax_summary %>% filter(triad_name == 'North' & comparison_years == '2016 vs. 2019'),
                         aes(x = sale_price_hpi, y = av_ratio_mailed, 
                             group = assessor_term_year, fill = assessor_term_year, color = assessor_term_year)) +
    facet_wrap( ~ reporting_group) +
    geom_hline(yintercept = .1, color = 'grey', alpha = .8, linetype=2) +
...
(south_comp_av <- ggplot(data = sales_ptax_summary %>% filter(triad_name == 'South' & comparison_years == '2017 vs. 2020'),
                         aes(x = sale_price_hpi, y = av_ratio_mailed, 
                             group = assessor_term_year, fill = assessor_term_year, color = assessor_term_year)) +
    facet_wrap( ~ reporting_group) +
    geom_hline(yintercept = .1, color = 'grey', alpha = .8, linetype=2) +
...
sales_ptax_summary <- sales_ptax %>% 
  ...
  summarize(count = sum(count), 
            ...
            sale_price_hpi = mean(sale_price_hpi)) %>%
  ungroup() %>%
  mutate(assessed_value_board_mailed_ratio = av_board_cpi/av_mailed_cpi,
  ...
  mutate(name_label = case_when(name == "count" ~ 'Count',
...
(bor_pre_post_chart <- ggplot(data = sales_ptax_summary %>% filter(name == 'assessed_value_board_mailed_diff'),
                              aes(x = sale_price_cpi, y = value, 
    ...
    scale_x_continuous(name = 'Sale price', expand = c(0,20), limits = c(0, 1000000), 
    ...
    theme_classic() + 
    theme(plot.subtitle = element_text(size = 12, hjust = .5, color = '#333333'),
          ...
          plot.margin = margin(t=15,r=20,b=10,l=15))  + 
    ggplot(data = sales_ptax_summary %>% filter(name == 'assessed_value_board_mailed_diff_pct'),
           aes(x = sale_price_cpi, y = value, 
               group = as.character(year), fill = as.character(year), color = as.character(year))) +
    geom_point(alpha = .2, aes(size = count)) + 
    geom_smooth(method = 'loess', span = 1, formula = 'y ~ x', aes(weight = count), se = FALSE) +
    ...
    theme(plot.subtitle = element_text(size = 12, hjust = .5, color = '#333333'),
          ...
          axis.text = element_text(size = 11, color = '#333333'), axis.title = element_text(size = 11, color = '#333333'),
          plot.margin = margin(t=15,r=20,b=10,l=15)) +
    plot_annotation(title = 'Change in assessed value of residential properties after Board of Review appeal',
                    caption = paste0('Note: the above data reflects reductions in assessed values among residential properties sold the same year for which they were re-assessed. Points are scaled to the number of sales.\n',
...
ptax_data_composition <- ptax_data %>%
  ...
  mutate(name = case_when(name == 'tax_bill_share' ~ 'Tax bills',
...
(res_composition_chart <- ggplot() +
  ...
  geom_point(data = ptax_data_composition %>% filter(res_nonres_label == 'Residential'),
             aes(x = year, y = value, color = name), size = 12, alpha = .9) +
  geom_line(data = ptax_data_composition %>% filter(res_nonres_label == 'Residential'),
            aes(x = year, y = value, color = name), linewidth = 2, alpha = .3) +
  geom_text(data = ptax_data_composition %>% filter(res_nonres_label == 'Residential'),
  ...
  labs(subtitle = 'Residential share of assessed property value and tax burden', 
...
bg_data <- get_acs(year = 2020, geography = "block group", 
...
ptax_data_geo <- ptax_data_geo %>% st_as_sf(coords = c("longitude", "latitude"),
...
sales_ptax_bg <- sales_ptax %>%
  ...
  filter( (triad_name == 'City' & comparison_years == '2018 vs. 2021') | 
          (triad_name == 'North' & comparison_years == '2016 vs. 2019') | 
          (triad_name == 'South' & comparison_years == '2017 vs. 2020') ) %>%
  mutate(av_ratio_mailed = av_mailed_hpi/sale_price_hpi,
...
(city_map <- ggplot() +
    geom_sf(data = sales_ptax_bg %>% filter(assessor_term == "Berrios", triad_name == 'City'),
    ...
    theme_void() +
    ggplot() +
    geom_sf(data = sales_ptax_bg %>% filter(assessor_term == "Kaegi", triad_name == 'City'),
            aes(fill = av_ratio_mailed), color = '#333333', linewidth = .1) +
...
(north_map <- ggplot() +
    geom_sf(data = sales_ptax_bg %>% filter(assessor_term == "Berrios", triad_name == 'North'),
            aes(fill = av_ratio_mailed), color = '#333333', linewidth = .1) +
    ...
    theme_void() +
    ggplot() +
    geom_sf(data = sales_ptax_bg %>% filter(assessor_term == "Kaegi", triad_name == 'North'),
            aes(fill = av_ratio_mailed), color = '#333333', linewidth = .1) +
...
(south_map <- ggplot() +
    geom_sf(data = sales_ptax_bg %>% filter(assessor_term == "Berrios", triad_name == 'South'),
            aes(fill = av_ratio_mailed), color = '#333333', linewidth = .1) +
    ...
    theme_void() +
    ggplot() +
    geom_sf(data = sales_ptax_bg %>% filter(assessor_term == "Kaegi", triad_name == 'South'),
            aes(fill = av_ratio_mailed), color = '#333333', linewidth = .1) +
